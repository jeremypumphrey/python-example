...
2) Run `pip list outdated --path ./` to identify versions with upgrades available
  
3) Run `pip-compile --output-file=requirements.txt --generate-hashes requirements.in` to create `requirements.txt` with specific version locks and SHA package validation. Never edit manually.  
> numpy==2.3.5 \  
